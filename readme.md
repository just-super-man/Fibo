...
3. Передача данных от 1 к 2 идет через REST API
4. Передача данных от 2 к 1 идет посредством брокера сообщений (например, RabbitMQ).
5. Язык C#, Платформа .Net Core